...
```mermaid
graph TD
    SM["StrictMode"] --> AP["AppProvider (context)"]
    AP --> A["App"]
    A --> EB["ErrorBoundary (organism)"]
    EB --> HP["HomePage (page)"]
    HP --> ML["MainLayout (template)"]

    ML -->|"header slot"| H["Header (organism)"]
    H --> TP["ThemePicker (molecule)"]
    TP --> BS["buttons, select"]
    H --> SI["SearchInput (atom)"]
    H --> SD["SelectDropdown × 7 (atom)"]
    H --> RBtn["Button — Reset (atom)"]

    ML -->|"tabs slot"| ST["SectionTabs (organism)"]
    ST --> MT["MemoTab × N"]
    MT --> STb["SectionTab (atom)"]

    ML -->|"children slot"| CH{"one of"}
    CH -->|loading| SP["Spinner (atom)"]
    CH -->|error| ES1["EmptyState (atom)"]
    CH -->|ready| MvT["MovieTable (organism)"]

    MvT --> THC["TableHeaderCell × N (molecule)"]
    THC --> SA["SortArrow (atom)"]
    MvT --> MR["MovieRow × N — virtual (molecule)"]
    MR --> BG["Badge × 1–2 (atom)"]
    MR --> RtB["RatingBar (atom)"]
    MvT --> ES2["EmptyState — 0 results (atom)"]

    classDef ctx fill:#e8f5e9,stroke:#2e7d32
    classDef page fill:#e3f2fd,stroke:#1565c0
    classDef org fill:#fff3e0,stroke:#e65100
    classDef mol fill:#f3e5f5,stroke:#7b1fa2
    classDef atom fill:#fce4ec,stroke:#c62828
    classDef tmpl fill:#e0f7fa,stroke:#00838f

    class AP ctx
    class HP page
    class EB,H,ST,MvT org
    class TP,THC,MR mol
    class SI,SD,RBtn,STb,SP,ES1,ES2,SA,BG,RtB atom
    class ML tmpl
```
...
```mermaid
flowchart TD
    subgraph DOMAIN["DOMAIN — pure functions, no side-effects"]
        direction TB
        MD["movie_list.md"] -->|parse| ME["MovieEntry[]"]
        POL["Policies\nfilter · rating · debounce · vScroll"]
        SEL["Selectors\ngenres · years · ratings · counts"]
        STR["Strategies\nsearch · sort"]
        CON["Contracts\nassertions · branded types"]
        FSM2["FSM\nidle · loading · ready · error"]
        EVB["EventBus"]
        FF["Feature Flags"]
    end

    subgraph APP["APP — imperative shell, I/O & wiring"]
        direction TB
    ...
    subgraph UIL["UI — React · Atomic Design"]
        direction TB
        APROV["AppProvider\n(React context)"]
        HP2["HomePage\n(page — data hub)"]
        HKS["Hooks\nuseMovieData · useFilters · useSort\nuseWatched · useTheme · useLiveAnnouncer"]
        HDR2["Header"]
        ST2["SectionTabs"]
        MT2["MovieTable"]
        ATM["Atoms & Molecules"]
    ...
    POL & SEL & STR & EVB & FF --> CR
    CR --> ADP & REPO & CMD & EL
    CR --> APROV
    HKS -.->|inject| HP2
    APROV --> HP2
    HP2 -->|"props ▼"| HDR2 & ST2 & MT2
    HDR2 & ST2 & MT2 -->|"callbacks ▲"| HP2
    HDR2 & MT2 --> ATM
```
...
stateDiagram-v2
    ...
    Loading --> DataError : fetch failed

    Ready --> EmptyResults : filters match 0 entries
    EmptyResults --> Ready : filters adjusted

    state Loading {
        [*] --> spin
        spin : Spinner 40×40 ring
        spin : "Loading movie data…"
        spin : No section tabs shown
    }

    state Ready {
        [*] --> table
        table : Section tabs with counts
        table : MovieTable with virtual scroll
        table : 9 columns — responsive hiding
    }

    state DataError {
        [*] --> errMsg
        errMsg : ⚠ EmptyState component
        errMsg : "Unable to load movie data"
        errMsg : No section tabs shown
    }

    state EmptyResults {
        [*] --> noMatch
        noMatch : EmptyState component
        noMatch : "No entries match your filters"
        noMatch : Section tabs still visible
    }

    note right of Loading : Header visible with "0 of 0"
    note right of DataError : Header visible with "0 of 0"
    note left of EmptyResults : Header shows "0 of N"
```

> **ErrorBoundary** — catches uncaught render exceptions at any point and replaces the
> entire viewport with a "Something went wrong" heading, the error message, a `<pre>`
> stack-trace box, and a **Reload Page** button.
...
block-beta
    ...
    block:HEADER["Header (sticky, gradient)"]
        columns 2
        TITLE["🎬 Movies for Hackers"]
        THEME["💻 System | 🌙 Dark | ☀️ Light | ▾ Palette"]
    end

    block:CONTROLS
        columns 4
        SEARCH["⌕ Search (/)"]
        FILTERS["▾ Genres | ▾ Formats | ▾ MPAA"]
        RANGES["Year ▾Min – ▾Max | IMDb ▾Min – ▾Max"]
        META["Reset | 142 of 350"]
    end

    block:TABS["Section Tabs (flex row, wrapping)"]
        columns 7
        T1["All 350"]
        T2["Thrillers 73"]
        T3["Sci-Fi 167"]
    ...
    block:TABLE["MovieTable (virtual scroll, 42 px rows, 10 overscan)"]
        columns 9
        C1["☐"]
        C2["Title 19%"]
        C3["Section 9%"]
...
| Viewport | Visible Columns | Hidden | Other Changes |
|:---|:---|:---|:---|
| **> 900 px** | ☐ Title Section Genre Format Year Rated Rating Description | — | Full padding, full font size |
